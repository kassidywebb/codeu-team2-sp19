...
public class Datastore {
	...
	public void storeEvent(Event event) {
		...
		eventEntity.setProperty("host", event.getHostedBy());
	...
	public void saveEventInformation(List<Event> events, PreparedQuery results) {
		...
		for (Entity entity : results.asIterable()) {
			try {
				...
				String hostedBy = (String) entity.getProperty("host");
				String imageUrl = (String) entity.getProperty("imageUrl");

				Event event = new Event(id, eventId, user, title, date,
						time, timestamp, location, details, hostedBy, imageUrl);
				events.add(event);
